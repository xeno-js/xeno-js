...
export class DrizzleSoftDeleteDataSource<T> extends AbstractDrizzleDataSource<T> {
  ...
  protected async performDelete(where: SQL): Promise<void> {
    await this._db.update(this._tableSchema).set({ is_deleted: true }).where(where)
  }
}
