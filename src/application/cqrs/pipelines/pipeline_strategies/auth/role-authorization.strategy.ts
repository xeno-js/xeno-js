 ...
 */
export class RoleAuthorizationStrategy extends BaseAuthorizationStrategy<ISecureCommand> {
  ...
  public isApplicable(command: ISecureCommand): command is ISecureCommand {
    return !Guards.isNullOrEmpty(command.requiredRoles)
  }
  ...
  protected performAuthorizationCheck(
    ...
    const hasRequiredRole = command.requiredRoles?.some((role) => roles.includes(role))
