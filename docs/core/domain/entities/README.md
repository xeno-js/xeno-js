...
lifecycle, spanning across multiple system-state mutations. Within the Gear5
...
`number`) to model entity identities. Gear5 wraps values inside a dedicated
...
```typescript
import { Entity, UniqueId } from '@gear5/core'
import type { Optional } from '@gear5/shared'
...
export class Customer extends Entity<CustomerProps> {
  ...
  public deactivateAccount(): void {
    ...
    // Inside Gear5, props are frozen on construction to lock state stability:
