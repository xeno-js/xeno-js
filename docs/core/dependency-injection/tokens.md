...
An injection token is the stable key used by the Gear5 service container to
...
Gear5 exposes framework tokens through `INJECTION_TOKENS`. For example, when you
call `builder.addDb(...)`, Gear5 registers the database client under:
